...
- **频繁切换抖动**:同文件在会话结束后 N 分钟(初值 3min,代码常量)内重新激活,
  且无其他文件的会话插入其间 → 视为续会话(沿用原缓冲与起止时间),避免
  "对照抄代码在两文件间来回切"把会话剁碎。实现时验证该规则的复杂度收益比,
  若过于复杂则退化为"接受碎片化 + L2 focus 注明短会话常见"。
...
4. **k=5 语义统一**:当前 L3 prompt 把 5 用作"No evidence / unaware"且"无法
   ...
   本 spec 接受现状并在 L2 focus 明确"无证据≠薄弱";底层语义统一归 mastery-fix
   轮的 deferred 项跟踪。
